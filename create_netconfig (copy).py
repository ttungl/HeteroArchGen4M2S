...
def create_netconfig(num_of_nodes, L2_blocksize, network_mode, LOCAL_LINKS_PATH, HYBRID_LINKS_PATH, LOCAL_LINKWIDTH, HYBRID_LINKWIDTH):
	...
	if network_mode == 1:
		...
		# for (index,eachcoord) in enumerate(array3):
		# 	print index,eachcoord
		##################################################
		...
		# print array4
		##################################################
		# 1. Find the shortest path from `src` to `dst` using dijkstra.
		...
		########################################
			...
			# graph = []
			# count = 0
			# for (src, dst) in array2:
			# 	count = count + 1
			# 	if src == prev: ## 
			# 		temdst.append(dst)
			# 	if not src == prev:
			# 		# print prev, temdst
			# 		graph.extend([[prev, temdst]])
			# 		temdst = [] ## reset
			# 		temdst.append(dst) ## next
			# 		prev = src ## update
			# 	if count==len(array2):
			# 		graph.extend([[prev,temdst]])
			# # print graph
		...
		# print gx.find_path(0,9)
		# print gx.find_all_paths(0,14)
		# 2. 
		for (s,d) in array4:
			# print s,d
